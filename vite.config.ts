...
/**
 * `.figma/make/site.json` is managed by the Figma Make platform and isn't
 * committed to the repo. Fall back to an empty configuration so local dev,
 * CI, and Vercel builds work without it.
 */
function loadSiteConfiguration(): FigmaSiteConfiguration {
  try {
    return JSON.parse(readFileSync(path.resolve(__dirname, '.figma/make/site.json'), 'utf-8'))
  } catch {
    return {}
  }
}
...
function figmaSiteConfiguration(config: FigmaSiteConfiguration): Plugin {
  ...
  const title = config.title ?? "English Grammar in Use"
