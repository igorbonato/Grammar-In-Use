...
```
design/           Aplicação front-end (Vite + React + Tailwind)
  src/
...
src/pdf/          Material bruto original (PDFs do livro), usado como fonte para
...
## Sobre os PDFs em `src/pdf/`

Os PDFs do livro (módulos gramaticais, additional exercises, appendix, etc.) são material de origem para a ingestão de conteúdo, mas **não são versionados no Git** — ficam listados no `.gitignore` por serem conteúdo protegido por direitos autorais e por inflarem desnecessariamente o repositório. Eles precisam existir localmente em `src/pdf/` para os scripts de ingestão funcionarem, mas cada colaborador deve obtê-los por fora do Git.
